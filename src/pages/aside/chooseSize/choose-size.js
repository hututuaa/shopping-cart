...
class ChooseSize extends React.Component {
    ...
    render() {
        ...
        for (let i = 0; i < 7; i++) {
            newSize.push(<Tooltip title={sizeList[i]} color="white" key={(sizeList[i].id, i)}><Button onClick={this.select} style={{ margin: 5 }} key={(sizeList[i].id, i)} shape="circle" size="large" >{sizeList[i]}</Button></Tooltip>);
        }
